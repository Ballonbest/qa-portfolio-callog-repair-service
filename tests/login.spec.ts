...
const goto_CustomerService = "https://samart-calllog.netserviceth.com/signin?redirect=/admin";
...
test.describe("Login - General", () => {
  test("TC_LOGIN_NEG_001 - Login ไม่สำเร็จเมื่อ user ไม่มีในระบบ", async ({ page }) => {
...
test.describe("Login - IT Support", () => {
  test("TC_LOGIN_POS_002 - IT Support Login สำเร็จ", async ({ page }) => {
  ...
  test("TC_LOGIN_NEG_003 - IT Support Password ไม่ถูกต้อง", async ({ page }) => {
    await page.getByPlaceholder(locatorEmail).fill(userItsupport!);
    await page.getByPlaceholder(locatorPassword).fill("invalid123");
    await page.getByRole("button", { name: "Sign in" }).click();
